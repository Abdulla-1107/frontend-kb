...
const Services = () => {
  ...
  const [selectedService, setSelectedService] = useState<'gate' | 'door' | 'fence' | 'railing' | 'restoration' | undefined>();
  ...
  const services = [
    {
      ...
      features: language === 'uz' 
        ? ['Avtomatik va oddiy', 'Profil temir va shveller', 'Pultdan boshqarish', 'Bo\'yash kiritilgan']
        : language === 'en'
        ? ['Automatic and manual', 'Profile metal and channel', 'Remote control', 'Painting included']
        : ['Автоматические и обычные', 'Профильный металл и швеллер', 'Пульт управления', 'Покраска включена'],
    },
    {
      id: 'door' as const,
      icon: DoorClosed,
      ...
      image: doorImage,
      features: language === 'uz'
        ? ['Xavfsiz qulflar', 'Individual dizayn', 'Rangli bo\'yoq', 'O\'rnatish kiritilgan']
        : language === 'en'
        ? ['Secure locks', 'Custom design', 'Color finish', 'Installation included']
        : ['Надёжные замки', 'Индивидуальный дизайн', 'Цветная покраска', 'Установка включена'],
    },
    {
      id: 'fence' as const,
      icon: Fence,
      ...
      image: fenceImage,
      features: language === 'uz'
        ? ['12x12 va 14x14 kvadrat temir', 'Dekorativ naqshlar', 'Himoya funksiyasi', 'Reshotkalar']
        : language === 'en'
        ? ['12x12 and 14x14 square metal', 'Decorative patterns', 'Protective function', 'Grilles']
        : ['Квадрат 12x12 и 14x14', 'Декоративные узоры', 'Защитная функция', 'Решётки'],
    },
    {
      id: 'railing' as const,
      icon: RailingIcon,
    ...
    },
    {
      ...
      image: doorImage,
      features: language === 'uz'
        ? ['Eski buyumlarni tiklash', 'Qayta bo\'yash', 'Yangiday ko\'rinish', 'Uyda yoki ishda']
        : language === 'en'
        ? ['Restoring old products', 'Repainting', 'Like-new appearance', 'At home or work']
        : ['Восстановление старых изделий', 'Перекраска', 'Вид как новый', 'На дому или на работе'],
    },
  ];

  const handleOrder = (serviceId: 'gate' | 'door' | 'fence' | 'railing' | 'restoration') => {
